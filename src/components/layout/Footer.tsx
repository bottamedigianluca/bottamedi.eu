...
const Footer: React.FC<FooterProps> = ({ language }) => {
  ...
  // CORREZIONE: Funzione per gestire click sui documenti legali
  const handleLegalDocumentClick = (docType: 'privacy' | 'terms') => {
    // Scroll al footer dove si trovano i documenti legali
    const legalElement = document.getElementById('legal-documents')
    if (legalElement) {
      legalElement.scrollIntoView({ behavior: 'smooth' })
      
      // Trigger per aprire il documento specifico (se c'è un sistema di modal/accordion)
      setTimeout(() => {
  ...
  return (
    <>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              ...
              <div className="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-6">
                <div className="flex space-x-6 text-sm text-white/60">
                  {/* CORREZIONE: Link funzionanti per documenti legali */}
                  <button 
      ...
      {/* NUOVO: Sezione Documenti Legali */}
      <div id="legal-documents">
        {/* Qui verrà incluso il componente LegalDocuments */}
      </div>
    </>
  )
}
