...
export default PremiumMobileDockimport React, { useState, useEffect, useCallback } from 'react'
...
// 🚀 HOOK SCROLL DETECTION COME ORIGINALE - SEMPLIFICATO
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      animationFrame = requestAnimationFrame(() => {
        ...
        const isLegalOpen = document.getElementById('legal-documents')?.style.display !== 'none' && 
                          !document.getElementById('legal-documents')?.classList.contains('hidden')
        ...
        if (isInHero || isInContact || isLegalOpen) {
          ...
          if (direction === 'up') {
            setIsVisible(true) // Scroll up = mostra subito
          } else {
            setIsVisible(false) // Scroll down = nascondi
            // Ma riapri dopo 1 secondo di inattività
            clearTimeout(inactivityTimer)
            inactivityTimer = setTimeout(() => {
              // Ricontrolla la sezione prima di mostrare
              const currentEl = document.getElementById(foundSection)
              if (currentEl && foundSection !== 'hero' && foundSection !== 'contact') {
                setIsVisible(true)
              }
            }, 1000)
          }
